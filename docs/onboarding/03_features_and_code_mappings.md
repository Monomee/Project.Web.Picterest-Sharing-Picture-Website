...
### 1. Narrative Description
Users can report content that violates community guidelines. If a post receives 5 or more unique user reports, it is automatically shadow-banned (its `DeliveryStatus` is set to `"hidden"`, removing it from the public feed). To prevent self-reporting loop abuse, a strict self-reporting constraint is enforced: a user is prohibited from reporting their own post. This rule is validated on the backend via an explicit exception block and the report trigger is hidden from the UI when a post belongs to the authenticated user. Admins review reported posts and can either delete them (using programmatic cascade deletes wrapped in a secure transaction), ban users (evicting their active status from the distributed Redis cache), or dismiss reports (restoring the post's status back to `"pending"`). All admin resolutions are logged to the database. Administrators can access the `/admin/logs` UI dashboard, which fetches paginated logs using TanStack Query.
...
sequenceDiagram
    ...
    alt User Reporting & Shadow-Ban Loop
        ...
        DetailPage->>ReportModal: Trigger modal (mounted=true)
        ReportModal->>Body: Breakthrough render via React Portal (createPortal)
        ...
        ReportsSvc->>Database: Query post details & reporter checks
        alt Self-Reporting Guard (reporterId == post.UserId)
        ...
        else Duplicate Report Check
            ReportsSvc-->>ReportsCtrl: Throw InvalidOperationException (Already reported)
            ReportsCtrl-->>ReportModal: Return HTTP 400 Bad Request
            ReportModal-->>User: Show duplicate warning Toast
        ...
        alt Action: DELETE_POST (Transaction & Eventual Consistency)
            AdminSvc->>Database: BeginTransactionAsync()
            AdminSvc->>Database: Programmatic CASCADE delete comments, likes, other reports
            AdminSvc->>Database: Remove Post entity
            AdminSvc->>Database: SaveChangesAsync() & CommitAsync()
            Note over AdminSvc,Database: SQL Transaction committed successfully
            AdminSvc->>MediaSvc: Async DeleteImageAsync(cloudinaryPublicId) via Task.Run()
            MediaSvc-->>AdminSvc: Cloud asset destruction dispatched
        else Action: BAN_USER (DB Save & Cache Eviction)
            AdminSvc->>Database: Set target user.Status = 'banned'
            AdminSvc->>Database: SaveChangesAsync()
            AdminSvc->>Redis: RemoveAsync("user-status:{targetUserId}") (Cache Eviction)
        else Action: DISMISS
            AdminSvc->>Database: Set post.DeliveryStatus = 'pending' & report.Status = 'dismissed'
            AdminSvc->>Database: SaveChangesAsync()
        end
    ...
    alt Admin Logs View Loop (/admin/logs)
        Admin->>AdminPage: Navigate to /admin/logs
        AdminPage->>AdminService: getAuditLogs(page, pageSize)
        AdminService->>AdminCtrl: GET /api/admin/logs?page=N&pageSize=M [Admin Guard JWT]
        AdminCtrl->>AdminSvc: GetAuditLogsAsync(page, pageSize)
        AdminSvc->>Database: Fetch audit logs (Skip/Take pagination) with Actor profiles (Username)
        Database-->>AdminSvc: Return paginated audit logs list
...
| Layer/Side | File Path | Technical Role & Responsibility in the Flow |
| :--- | :--- | :--- |
| Frontend | [ReportPostModal.tsx](file:///d:/Dev_Web/Picterest/frontend/src/components/pinterest/ReportPostModal.tsx) | Renders report forms inside React Portal (`createPortal`), bypassing relative layout CSS stack traps. Uses `mounted` state hooks for hydration safety. |
| Frontend | [page.tsx](file:///d:/Dev_Web/Picterest/frontend/src/app/admin/reports/page.tsx) | Admin reports queue dashboard. Renders pending reports and dispatches resolution actions. |
| Frontend | [page.tsx](file:///d:/Dev_Web/Picterest/frontend/src/app/admin/logs/page.tsx) | Renders the paginated administrative audit logs dashboard view (`/admin/logs`). |
| Frontend | [admin.service.ts](file:///d:/Dev_Web/Picterest/frontend/src/services/admin.service.ts) | Triggers API calls to fetch logs/reports and contains UI type signatures. |
| Backend API | [ReportsController.cs](file:///d:/Dev_Web/Picterest/backend/SharingPicture/SharingPicture.WebApi/Controllers/ReportsController.cs) | Exposes the JWT-protected `POST /api/reports` endpoint. |
| Backend API | [AdminController.cs](file:///d:/Dev_Web/Picterest/backend/SharingPicture/SharingPicture.WebApi/Controllers/AdminController.cs) | Exposes resolution POST and audit logs GET endpoints guarded by role requirements. |
| Backend WebApi | [UserStatusValidationMiddleware.cs](file:///d:/Dev_Web/Picterest/backend/SharingPicture/SharingPicture.WebApi/Middleware/UserStatusValidationMiddleware.cs) | Middleware validating active user status from Redis cache (`IDistributedCache`) with SQL Server fallback. |
| Backend Services | [ReportService.cs](file:///d:/Dev_Web/Picterest/backend/SharingPicture/SharingPicture.Services/ReportService.cs) | Performs reporter validation guards to reject self-reporting and duplicate submissions. |
| Backend Services | [AdminService.cs](file:///d:/Dev_Web/Picterest/backend/SharingPicture/SharingPicture.Services/AdminService.cs) | Drives moderation resolving (wrapping deletes in explicit transaction blocks, evicting status caches on ban, and dispatching async Cloudinary purges). |
| Backend Services | [MediaService.cs](file:///d:/Dev_Web/Picterest/backend/SharingPicture/SharingPicture.Services/MediaService.cs) | Connects to external Cloudinary instances via `CloudinaryDotNet` to delete assets asynchronously. |
| Backend Services | [AuditLogService.cs](file:///d:/Dev_Web/Picterest/backend/SharingPicture/SharingPicture.Services/AuditLogService.cs) | Creates historical audit log records during administrative actions. |

### 4. End-to-End Data Flow Explanation
When a report is submitted, `ReportService.ReportPostAsync` queries the `Post` record to verify the owner of the post is not the reporter. If `post.UserId == reporterId`, it throws an `InvalidOperationException` returning HTTP 400 Bad Request. When validation checks succeed, the report is saved. When an administrator resolves reports:
- **`DELETE_POST`**: All database removals are wrapped in an explicit transaction block (`BeginTransactionAsync`). Database comments, likes, other reports, and the post itself are deleted. Once changes are successfully written and committed (`SaveChangesAsync` and `CommitAsync`), the system schedules the Cloudinary asset deletion asynchronously via `MediaService.DeleteImageAsync` inside a background `Task.Run` call. If the SQL transaction fails or rolls back, the cloud file is preserved, preventing distributed state mismatch.
- **`BAN_USER`**: Sets the target user's status to `"banned"` in SQL Server and immediately evicts the cached status key `user-status:{userId}` from Redis (`IDistributedCache.RemoveAsync`). On subsequent requests, the `UserStatusValidationMiddleware` catches the ban immediately because the cache is empty, queries SQL Server, and returns HTTP 401 Unauthorized, bypassing Kestrel routing.
- **`DISMISS`**: Sets `DeliveryStatus` to `"pending"`, restoring post visibility.
All actions are logged to the `audit_logs` table. In the frontend, administrators access `/admin/logs` which fetches paginated data, and the `ReportPostModal` breakout uses React Portal to render inside `document.body` to guarantee absolute positioning and avoid parent scale traps.

> [!NOTE]
> **React Portal Viewport Breakthrough**:
> The `ReportPostModal.tsx` component is wrapped with React's `createPortal` targeting `document.body`. This forces the modal layout to render outside the DOM hierarchy of parent elements (like `PinCard` or other grid blocks). This technique safely breaks out of parent CSS transforms (like grid card hover scaling) and overflow clips, resolving viewport trapping bugs and guaranteeing absolute z-index positioning on the screen.
